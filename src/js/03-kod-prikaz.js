...
const KW = {
  ...
  js: 'async|await|break|case|catch|class|const|continue|default|delete|do|else|export|extends|false|finally|for|function|if|import|in|instanceof|let|new|null|of|return|super|switch|this|throw|true|try|typeof|undefined|var|while',
...
// Tab = jedan nivo uvlačenja (Python 4 razmaka, JavaScript 2, kao u inhome); Enter zadrži uvlačenje
// i doda nivo poslije ':' (Python) ili '{', '(' i '[' (JavaScript).
function wireEditor(id, lang) {
  const ta = document.getElementById(id), g = document.getElementById(id + '-g');
  const js = lang === 'js' || lang === 'dom';
  const nivo = js ? '  ' : '    ';
  ...
  ta.addEventListener('keydown', e => {
    ...
    if (e.key === 'Tab' && !e.shiftKey) { e.preventDefault(); ta.setRangeText(nivo, a, b, 'end'); upd(); }
    else if (e.key === 'Enter') {
      const line = v.slice(v.lastIndexOf('\n', a - 1) + 1, a);
      let ind = (line.match(/^ */) || [''])[0];
      if ((lang === 'python' && /:\s*$/.test(line)) || (js && /[{(\[]\s*$/.test(line))) ind += nivo;
